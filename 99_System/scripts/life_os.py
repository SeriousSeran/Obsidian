...
REPORTS = ROOT / "99_System" / "reports"
...
REQUIRED_FOLDERS = [
    ...
    ".obsidian/snippets",
    "00_Inbox",
    "00_Inbox/Voice_Dumps",
    "00_Inbox/Ideas",
    "00_Inbox/Screenshots",
    "01_Daily/Daily_Notes",
    "01_Daily/Weekly_Reviews",
    "01_Daily/Monthly_Reviews",
    "01_Daily/Quarterly_Reviews",
    "02_Medicine",
    "02_Medicine/Topics",
    ...
    "08_Relationships",
    "08_Relationships/People",
    "09_Bucket_List",
    "09_Bucket_List/Experiences",
    "10_Maps_Of_Content",
    "90_Templates",
    "99_System/design",
    "99_System/git",
    "99_System/homepage",
    "99_System/learning_resources",
    "99_System/periodic_notes",
    "99_System/quickadd",
    "99_System/reports",
    "99_System/schemas",
    "99_System/scripts",
...
REQUIRED_DASHBOARDS = [
    "10_Maps_Of_Content/Life_OS_Home.md",
    "10_Maps_Of_Content/Task_Dashboard.md",
    "10_Maps_Of_Content/Learning_Resource_Hub.md",
    "10_Maps_Of_Content/Life_OS_Command_Center.canvas",
    "02_Medicine/Medicine_Dashboard.md",
    "03_Money/Money_Dashboard.md",
    "04_Mind/Mind_Dashboard.md",
...
REQUIRED_MOCS = [
    "10_Maps_Of_Content/Medicine_MOC.md",
    "10_Maps_Of_Content/Money_MOC.md",
    "10_Maps_Of_Content/Mind_MOC.md",
...
REQUIRED_LEARNING_RESOURCES = [
    "02_Medicine/Learning/Medicine_Learning_Library.md",
    "03_Money/Learning/Money_Learning_Library.md",
    "04_Mind/Learning/Mind_Learning_Library.md",
    "05_Body/Learning/Body_Learning_Library.md",
...
def classify_root_file(path: Path) -> tuple[str, str, str]:
    ...
    if "medicine" in name or "clinical" in name or "osce" in name or "exam" in name:
        return "02_Medicine/", "medical learning or clinical study", "high"
    if "money" in name or "finance" in name or "commerce" in name or "sales" in name:
        return "03_Money/", "money, commerce, or business planning", "high"
    if "content" in name or "writing" in name or "brand" in name:
        return "06_Content/", "content or publishing system", "high"
    if "project" in name or "product" in name or "idea" in name:
        return "07_Projects/", "project or product material", "medium"
    if "health" in name or "body" in name or "physique" in name:
        return "05_Body/", "body, health, or training material", "high"
    if "reflection" in name or "mind" in name or "journal" in name:
        return "04_Mind/", "reflection or mind material", "medium"
    if "person" in name or "relationship" in name or "people" in name:
        return "08_Relationships/", "relationship or people material", "medium"
    if "bucket" in name or "someday" in name or "dream" in name:
        return "09_Bucket_List/", "long-term goal or experience", "medium"
    if "template" in name:
        return "90_Templates/", "template material", "high"
    return "00_Inbox/", "unclear root note", "low"
...
def link_health(_args: argparse.Namespace) -> Path:
    ...
    for path in markdown_files():
        ...
        if path_text.startswith(orphan_exclusions):
            continue
        if key not in inbound and path_key not in inbound and "10_Maps_Of_Content" not in path_text:
...
def create_daily_note(_args: argparse.Namespace) -> Path:
    ...
    destination = ROOT / "01_Daily" / "Daily_Notes" / f"{today.isoformat()}.md"
    template = ROOT / "90_Templates" / "Daily_Note_Template.md"
...
def create_weekly_review(_args: argparse.Namespace) -> Path:
    ...
    destination = ROOT / "01_Daily" / "Weekly_Reviews" / f"{week}.md"
    template = ROOT / "90_Templates" / "Weekly_Review_Template.md"
    destination.parent.mkdir(parents=True, exist_ok=True)
    if not destination.exists():
...
def validate_notes(_args: argparse.Namespace) -> Path:
    ...
    for path in markdown_files():
        ...
        path_text = rel(path)
        if path_text.startswith(("02_Medicine/", "03_Money/", "04_Mind/")) and fm.get("review_needed") not in {"true", "false"}:
...
def process_voice_dumps(_args: argparse.Namespace) -> Path:
    ensure_folders()
    folder = ROOT / "00_Inbox" / "Voice_Dumps"
    processed_dir = ROOT / "04_Mind" / "Reflections"
    actions = []
    review = []
