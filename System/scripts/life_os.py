...
def link_health(_args: argparse.Namespace) -> Path:
    ...
    for path in markdown_files():
        ...
        for target in extract_obsidian_links(text):
            # Ignore report links, template syntax, placeholders
            if (
                len(path.parts) >= 2 and path.parts[0] == "System" and path.parts[1] == "reports"
                or "System/reports/" in target
                or "<%" in target and "%>" in target
                or "path/to/processed/note" in target
                or "SOURCE_NOTE" in target
            ):
                continue
