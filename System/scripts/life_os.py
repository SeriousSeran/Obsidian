    ...
    for path in markdown_files():
        ...
        for target in extract_obsidian_links(text):
            ...
            if not link_target_exists(target, index):
                if not (target.startswith("System/reports/") or "tp." in target or target == "path/to/processed/note" or target == "Inbox/Voice_Dumps/SOURCE_NOTE" or "<%" in target or "%>" in target):
