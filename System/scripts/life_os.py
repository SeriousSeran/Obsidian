...
def link_health(_args: argparse.Namespace) -> Path:
    ...
    for path in markdown_files():
        ...
        for target in extract_obsidian_links(text):
            # Ignore dynamic/template links
            if target.startswith("System/reports/") or target.startswith("<%") or target in ["path/to/processed/note", "SOURCE_NOTE", "Inbox/Voice_Dumps/SOURCE_NOTE"] or target.endswith("System/reports/link_health"):
                continue
            key = target.removesuffix(".md").lower()
            inbound[key] += 1
